...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("🔥 Fire Monitor Database Test")
    print("=" * 50)
    
    # Test 1: Insertar detección
    print("\n1️⃣ Insertando detección...")
    detection_id = db.insert_detection(detected=True, esp32_millis=12345)
    print(f"   ✓ Detección guardada (ID: {detection_id})")
    ...
    # Test 3: Guardar imagen
    print("\n3️⃣ Guardando registro de imagen...")
    image_id = db.save_image_record(
        alert_id=alert_id,
        file_path='/home/pi/fire_images/test.jpg',
        file_name='test.jpg',
        image_size=12345,
        width=320,
        height=240,
    ...
    print(f"   ✓ Imagen registrada (ID: {image_id})")
    
    # Test 4: Actualizar estado de dispositivo
    print("\n4️⃣ Actualizando estado de dispositivo...")
    ...
    stats = db.get_system_status()
    for key, value in stats.items():
        print(f"   • {key}: {value}")
    
    # Test 6: Obtener configuración
    print("\n6️⃣ Configuración:")
    print(f"   • Threshold: {db.get_config('alert_threshold')}")
    print(f"   • Auto-capture: {db.get_config('auto_capture_enabled')}")
    print(f"   • Retention: {db.get_config('image_retention_days')} días")
    
    # Test 7: Resolver alerta
    print("\n7️⃣ Resolviendo alerta...")
    db.resolve_alert(alert_id, status='RESOLVED')
    print("   ✓ Alerta resuelta")
    
    # Test 8: Logs recientes
    print("\n8️⃣ Últimos 5 logs:")
    logs = db.get_logs(limit=5)
    for log in logs:
        print(f"   [{log['log_level']}] {log['component']}: {log['message']}")
