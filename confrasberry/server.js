...
io.on('connection', (socket) => {
    ...
    // Consultar la alerta activa en la base de datos
    try {
        const db = require('better-sqlite3')('/home/pi/fire_monitor/fire_monitor.db');
        const activeAlert = db.prepare("SELECT * FROM alerts WHERE status = 'ACTIVE' ORDER BY created_at DESC LIMIT 1").get();
        systemStatus.fireActive = !!activeAlert;
        systemStatus.lastAlert = activeAlert ? {
            type: activeAlert.alert_type,
            timestamp: activeAlert.created_at,
            detections: activeAlert.detections_count || 0
        } : null;
    } catch (err) {
        console.error('Error consultando alerta activa:', err.message);
        systemStatus.fireActive = false;
        systemStatus.lastAlert = null;
    }
    ...
    // Manejar resolución de alerta desde la web
